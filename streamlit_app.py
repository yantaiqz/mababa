...
<style>
    ...
    .emoji-btn-div button:hover {{ transform: scale(1.1); }}
    .emoji-btn-div button:active {{ transform: scale(0.9); }}
...
for i in range(0, len(items), cols_per_row):
    # 外层列：左右留空布局
    outer_cols = st.columns([0.2, 1, 1, 1, 0.2]) # 中间3列放商品
    ...
    for j in range(cols_per_row):
        if i + j < len(items):
            ...
            with outer_cols[j+1]: 
                ...
                # 1. Emoji 按钮 (点击购买逻辑)
                st.markdown('<div class="emoji-btn-div">', unsafe_allow_html=True)
                # 使用 st.button 并配合 CSS 实现大图标点击
